		...
		if(defined(DB_HOST)){
			...
			if(!$link){
				...
				if($token){
					...
					if($row){
						return 2; // le token est déjà en base
					}
				}
			...
			fwrite($handle, 'ENVIRONMENT='.ENVIRONMENT.'#'.'MERCHANT_ID='.MERCHANT_ID."#CONTRACT=$contractNumber#ORDER_REF=$orderRef#TRANSACTION_ID=$transactionId#ACTION=$paymentAction#TOKEN=$token#AMOUNT=$amount#CURRENCY=$currency#DATE=".time()."#RESULT=$resultCode#\n");
			fclose($handle);
		}
	}
